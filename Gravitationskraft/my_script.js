...
function change_img_lection() {
  ...
  if (
    location.pathname == "/Gravitationskraft/gravitation.html" ||
    location.pathname == "/ampharos/Gravitationskraft/gravitation.html" ||
    location.pathname == "ampharos/Gravitationskraft/gravitation.html"
  ) {
    link_lection_list[0] = "gravitation.html#sticky1";
  }
...
function drop(ev) {
  ...
  if (ev.currentTarget.querySelectorAll("img").length > 0) {
    alert("Es gibt schon ein Zeichen hier");
  } else {
    var data = ev.dataTransfer.getData("text");
